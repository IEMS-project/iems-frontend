	...
	return (
		<div className="flex h-full flex-col p-4 bg-background">
					...
					<div className="space-y-3">
						{messages.map((m, i) => (
							<div key={i} className={`${m.role === 'user' ? 'text-right' : 'text-left'}`}>
								{m.file ? (
									<div className="inline-block max-w-[80%] rounded-2xl bg-blue-50 dark:bg-blue-900/30 p-3 text-left text-sm text-blue-900 dark:text-blue-100 shadow-sm border border-blue-100 dark:border-blue-800">
										...
										{m.file.isImage ? (
											...
											<a href={m.file.url} download className="mt-2 inline-block underline hover:text-blue-700 dark:hover:text-blue-300">Tải xuống</a>
										)}
									</div>
								) : (
									<span className={`inline-block max-w-[80%] rounded-2xl px-4 py-2 text-sm leading-relaxed shadow-sm ${m.role === 'user' ? 'bg-blue-600 dark:bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-700'}`}>
										{m.text}
									</span>
								)}
							</div>
			...
			<div className="mt-3 flex items-end gap-2">
				...
				<Button aria-label="Gửi tin nhắn" onClick={send} className="bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 text-white">Gửi</Button>
			</div>
		</div>
	);
}
